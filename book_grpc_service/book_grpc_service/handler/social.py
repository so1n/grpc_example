...
from book_grpc_service.helper.field import proto_load, repeat_to_list, timestamp_to_datetime
...
class SocialService(social_service.BookSocialServicer):
    ...
    def get_book_like(self, request: social_message.GetBookLikesRequest,
        ...
        return social_message.GetBookLikesListResult(result=proto_load(social_dal.get_book_likes(isbn=repeat_to_list(request.isbn))))
        ...
        return social_message.GetBookCommentListResult(
            result=proto_load(
                social_dal.get_book_comment(
                    ...
                    create_time=timestamp_to_datetime(request.next_create_time, default=None),
                    limit=request.limit
                )
            )
        )